...
const expenseSchema = z.object({
  ...
  category: z.string().describe("Category: groceries, dining, transport, shopping, travel, entertainment, bills, health, or other"),
});
...
async function testPDFExtraction(): Promise<void> {
  ...
  try {
    ...
    for await (const element of pdfElementStream) {
      pdfCount++;
      console.log(`PDF Transaction ${pdfCount}:`, JSON.stringify(element, null, 2));
    }
